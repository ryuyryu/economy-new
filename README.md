...
ゲーム画面上部には **ターン数・評価** が表示され、右上の「メニュー」ボタンで詳細情報ドロワーを開閉できます。
スタート画面の背景は少し暗めの画像を使用しているため、CSS の `brightness()` で明るさを調整しています。

スタート画面の背景は少し暗めの画像を使用しているため、CSS の `brightness()` 関数で明るさを調整しています。
...
※初回は `npm install` を実行して依存パッケージを導入してください。
