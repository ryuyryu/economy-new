...
1. `notifications.html` を開くと、通知が1枚に連結されたリストで表示されます。
   各項目は太い境界線で仕切られています。
2. 気になる通知をタップすると詳細ページに移動します。
3. 通知を右から左へスワイプすると「削除」ボタンが現れ、そのままタップで一覧から消せます。
4. 詳細ページ下部のアンケートに回答すると、結果を保存したあと自動的に一覧画面へ戻ります。
   このとき回答した通知は既読扱いとなり、影付きで表示されます。
